...
def match_hwinfo_to_epochs(hwinfo_df, training_data):
    ...
    if power_cols:     print(f"  ✓ Power column: {power_cols[0]}")
    if temp_cols:      print(f"  ✓ Temp column: {temp_cols[0]}")
    if mem_alloc_cols: print(f"  ✓ Memory allocated column: {mem_alloc_cols[0]}")
    if mem_total_cols: print(f"  ✓ Memory total column: {mem_total_cols[0]}")
